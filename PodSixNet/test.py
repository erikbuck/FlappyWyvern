...
class FailEndPointTestCase(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(str(self.endpoint_bad.result), str(
            want), "Socket got %s instead of %s" % (str(self.endpoint_bad.result), str(want)))
...
class EndPointTestCase(unittest.TestCase):
    def setUp(self):
        self.outgoing = [
            {"action": "hello", "data": {"a": 321, "b": [2, 3, 4], "c": [
                "afw", "wafF", "aa", "weEEW", "w234r"], "d": ["x"] * 256}},
        ...
        class ServerChannel(Channel):
            def Network_hello(self, data):
                ...
                self.Send({"action": "gotit", "data": "Yeah, we got it: " +
                           str(len(data['data'])) + " elements"})
    ...
    def runTest(self):
        ...
        self.failUnless(self.server.count == self.count,
                        "Didn't receive the right number of messages")
        self.failUnless(self.endpoint.count == self.count,
                        "Didn't receive the right number of messages")
...
class ServerTestCase(unittest.TestCase):
    testdata = {"action": "hello", "data": {"a": 321, "b": [2, 3, 4], "c": [
        "afw", "wafF", "aa", "weEEW", "w234r"], "d": ["x"] * 256}}
    ...
    def runTest(self):
        ...
        for x in range(250):
            ...
            sleep(0.001)
        self.failUnless(self.server.connected == True,
                        "Server is not connected")
        self.failUnless(self.outgoing.connected == True,
                        "Outgoing socket is not connected")
